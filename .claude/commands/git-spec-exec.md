...
依序尋找 spec 來源：
1. 根目錄 `git-worktree-spec.md`（尚未歸檔）→ 若存在，使用此檔
2. `history_specs/` 目錄下最新一筆（檔名排序最後）→ 若存在，使用此檔
3. **兩者皆無**：告知使用者此 worktree 沒有 spec，詢問是否手動描述需求或中止

> 若從 `history_specs/` 讀取，告知使用者「此 spec 已歸檔，來源：`<路徑>`」
...
1. 每完成一項任務，更新 spec 檔（無論是 `git-worktree-spec.md` 或歸檔路徑），將已完成項目打勾 `[x]`
